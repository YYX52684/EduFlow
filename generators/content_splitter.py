...
{{
  "stages": [
    {{
      ...
      "description": "学生可见的阶段介绍（仅说明本阶段在做什么，如「与患者见面并进行问诊与体格检查」，2-3句话；不写具体问诊要点、诊断结论、需鉴别的病种等会泄露答案的内容）",
    ...
    {{
      "id": 2,
      "title": "另一场景标题",
      "description": "学生可见的阶段介绍（同上，不泄露核心要点与答案）",
...
字段说明：
- title: 场景名称，用于显示在节点上
- description: **学生可见**的阶段介绍。只写本阶段在做什么（如「与患者见面并进行问诊与体格检查」「根据已有信息给出诊断并解释」），2-3句话。**严禁**在 description 中写出：具体问诊要点、必问项、诱因/主症清单、诊断结论、需鉴别的病种、关键检查结果等会泄露答案的内容，以免学生提前知道「核心问诊点」而失去练习价值
...
6. **description 面向学生**：只写「本阶段在做什么」的概括，不写学生需自行问出或推断出的要点（如具体诱因、诊断、鉴别病种等），避免泄露核心问诊点与答案
