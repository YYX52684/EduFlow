...
class CardASignature(dspy.Signature):
    ...
    role_section: str = dspy.OutputField(desc="# Role 部分：NPC是谁，背景、性格、说话方式。建议50-100字，简洁自然即可。用「你」指代 NPC，例如「你是张经理，食品厂设备采购经理……」。")
    context_section: str = dspy.OutputField(desc="# Context 部分：当前场景背景，对方（学生）扮演什么角色。建议50-100字。用「你」指 NPC、「学生/对方」指学生。若本环节紧接在角色切换之后（上一张A卡是不同NPC），须写明：开场第一句须含简短情境承接（如时间、场景或身份，例如「好的，我们到病房了」「术后第二天了」「护士，我们明天要出院了」），再进入本角色第一句台词，避免学生感到突兀。")
    interaction_section: str = dspy.OutputField(desc="# Interaction 部分：你（NPC）如何与学生/对方对话、推进剧情。建议80-150字。用「你」指 NPC、「学生/对方」指学生。重要：1）每轮只问1-2个问题，不要连续追问；2）严禁使用机械连接词如「你提到」「这两点直接关系到」「第一...第二...」等重复句式；3）学生回答正确时给予具体正向激励，如「这个思路很清晰」「数据支撑很好」等；4）若本环节为学生向NPC咨询（如家属问护士出院指导），须写明每个主题追问不超过1～2轮、对方说清关键步骤后表示理解并感谢，避免对同一细节重复追问超过2次。")
    transition_section: str = dspy.OutputField(desc="# Transition 部分：什么情况下触发场景切换。用「你」指 NPC、「学生/对方」指学生。必须写明：仅在学生达到本环节核心目标（如完成关键任务、达到最低要求）时才输出下一张卡片；若未完成，写明「不要输出卡片XB，可继续追问/引导……」再给出跳转指令。建议50-80字。")
    constraints_section: str = dspy.OutputField(desc="# Constraints 部分：你（NPC）扮演时的限制和注意事项。使用短横线列表格式。必须包含：1）每轮只问1-2个问题，每轮回复控制在350字以内；2）严禁使用以下机械连接词：「你提到」「这两点直接关系到」「第一...第二...」「哦？」等；3）学生回答正确时给予具体正向激励（如「很好」「思路清晰」「数据准确」），每2-3轮至少一次；4）若本环节为引导型（如带教护士、医生提问学生），须包含「学生答对后至少追问1次为什么/依据/还需要注意什么，再进入下一问或下一环节」；5）使用多样化的自然表达，避免重复句式。")
...
class CardBSignature(dspy.Signature):
    ...
    output_section: str = dspy.OutputField(desc="# Output 部分：过渡语模板或示例，建议30-80字。需体现「根据事实」：可写两种表述（肯定版/指出不足版）或通用指引。正文不要包含「无论您是否……」；可用「您在本环节完成了……现在请……」或先指出不足再衔接。不要第三人称场景描写。")
...
class CardBNarratorSignature(dspy.Signature):
    ...
    output_section: str = dspy.OutputField(desc="# Output 部分：过渡内容，建议50-80字。需体现「根据事实」；然后说明角色切换、介绍新角色。正文要简短、像场景提示，少用长段第三人称旁白（如「经过……现在请将注意力转向……」），以免打破沉浸感。可改为一句情境句（如「病房里，责任护士已备好报告」「术后第二天，患儿出现新情况」「病情稳定，母亲带着出院疑问等待沟通」）。不使用括号。")
    transition_section: str = dspy.OutputField(desc="# Transition 部分：仅包含跳转指令，格式为 **卡片XA** 或 **结束**")
...
class DSPyCardGenerator:
    ...
    def _truncate_text(self, text: str, max_len: int = 350) -> str:
