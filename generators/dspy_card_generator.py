...
class CardBSignature(dspy.Signature):
    ...
    context_section: str = dspy.OutputField(desc="# Context 部分：说明本过渡语的使用原则——根据上一环节实际表现，表现好则肯定，表现不足则简要指出，然后开启下一环节。1-2句话。")
    output_section: str = dspy.OutputField(desc="# Output 部分：过渡语模板或示例，30-80字。需体现「根据事实」：可写两种表述（肯定版/指出不足版）或一句通用指引（如「根据上一环节表现予以肯定或简要指出不足后，接下来进入……」）。不要第三人称场景描写。")
...
class CardBNarratorSignature(dspy.Signature):
    ...
    context_section: str = dspy.OutputField(desc="# Context 部分：说明本过渡语的使用原则——根据上一环节实际表现，表现好则肯定，表现不足则简要指出，然后衔接角色切换。1-2句话。")
    output_section: str = dspy.OutputField(desc="# Output 部分：过渡内容，50-80字。需体现「根据事实」：可写两种表述（肯定版/指出不足版）或通用指引；然后说明角色切换、介绍新角色。不使用括号。")
    transition_section: str = dspy.OutputField(desc="# Transition 部分：仅包含跳转指令，格式为 **卡片XA** 或 **结束**")
