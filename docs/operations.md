...
Web 支持：上传剧本并分析结构、生成教学卡片、运行自动模式模拟测试、评估会话日志；平台配置支持「从 URL 设置项目」一键填充课程/任务 ID；**6. Trainset 与 DSPy 优化** 支持构建 trainset、校验结构、运行 DSPy 优化（bootstrap/mipro）。手动/混合模式模拟仍请使用命令行。
