...
from api.workspace import list_projects, set_current_project, get_current_project

router = APIRouter()


@router.get("/list")
def list_projects_api(workspace_id: str = Depends(require_workspace_owned)):
    """列出当前工作区下所有项目（课程 + 小项目）。"""
    return {"projects": list_projects(workspace_id)}


@router.get("/current")
def get_current_project_api(workspace_id: str = Depends(require_workspace_owned)):
...
@router.put("/current")
def set_current_project_api(req: SetCurrentProjectRequest, workspace_id: str = Depends(require_workspace_owned)):
